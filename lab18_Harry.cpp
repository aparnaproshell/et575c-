...
Aparna Harry
Nov 28, 2025
Lab 18: 1D Arrays and Functions
This program collects up to 5 non-zero integers from the user,
calculates their average, finds the number closest to the average,
and displays the results using functions.
*/

#include <iostream>
#include "lab18_function.cpp"
#include <cmath>  // for abs() function to calculate absolute differences
using namespace std;

// Function prototypes
int collectnumber(int arr[]); // Collects numbers from user, returns count
double Averagenumber(int arr[], int count); // Calculates and returns average
int Closestmean(int arr[], int count, double avg); // Finds number closest to average
void Printresult(double avg, int closest); // Displays the final result

int main() {
    // Declare array to hold up to 5 integers
    int numbers[5];
    int count = 0; // Variable to track how many numbers were entered
    double average = 0.0; // Variable to store the calculated average
    int closestNumber = 0; // Variable to store the number closest to average
    
    cout << "\n ----- Lab 18: 1D Arrays and Functions -----" << endl;
    cout << "This program collects up to 5 non-zero integers." << endl;
    cout << "Enter 0 to stop early.\n" << endl;
    
    // Step 1: Collect numbers from user
    count = collectnumber(numbers);
    
    // Check if any numbers were entered
    if (count == 0) {
        cout << "\nNo numbers were entered. Program ending." << endl;
        return 0; // Exit program if no numbers collected
    }
    
    // Step 2: Calculate the average of entered numbers
    average = Averagenumber(numbers, count);
    
    // Step 3: Find the number closest to the average
    closestNumber = Closestmean(numbers, count, average);
    
    // Step 4: Display the results
    Printresult(average, closestNumber);
